...
export function changePreEngagementPhase({ phase }: { phase: PreEngagementFormPhase }) {
    return {
        type: ACTION_CHANGE_PRE_ENGAGEMENT_PHASE,
        payload: {
            currentPreEngagementPhase: phase
        }
    };
}

export function updateNewBookingData(data: Partial<NewBookingData>) {
    return {
        type: ACTION_UPDATE_PRE_ENGAGEMENT_NEW_BOOKING_DATA,
        payload: {
            ...data
        }
    };
